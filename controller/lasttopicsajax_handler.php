...
class lasttopicsajax_handler
{
	...
	*/
    ...
    {
        print_r("get_forums_by_column:num = " . $num);
        $col_num = $this->config['lasttopicsajax_ids_column' . $num];
        $col_name = 'lasttopics ajax_ids_column' . $num;
        ...
        if ($this->config[$col_name])
        {
         ...
         if ($with_subforums && $this->config[$col_name] != '*')
        {
            foreach ($col_forums as $fid)
            {
//               $sql = 'SELECT forum_id  FROM ' . FORUMS_TABLE . ' WHERE parent_id =' . $fid;
...
//               $sql = 'SELECT forum_id  FROM ' . FORUMS_TABLE . ' WHERE parent_id =' . $fid;
 ...
 //************               
        ...
        //        $sql_array_join = $sql_array['LEFT_JOIN'];
                //print_r($sql_array_join);
        //        if (1)
        //        {
        //            $sql_array_join = array_merge($sql_array_join,
        //                        array(
        //                                'FROM'	=> array(POSTS_TABLE => 'fp'),
        //                                'ON'	=> 'fp.post_id = t.topic_first_post_id'
        //                                )
        //                );
        //        }
        //$sql_array['LEFT_JOIN'] = $sql_array_join;
        //print_r($sql_array);
        //print_r('sql123 = ' . $this->db->sql_build_query('SELECT', $sql_array));
			    /**
            ...
            //    if ($num == 0)
            //    {
            ////print_r('sql = ' . $this->db->sql_build_query('SELECT', $sql_array));
            //    }
            //print_r('$howmany = ' . $howmany);
    ...
    public function last_col_markread()
    {
        ...
        ////print_r($rootref);
        //$pagination_block = $this->build_pagination($col_num, $rootref, $dataref);
        //$topics_block = $this->build_topics($rowset);
        $data = array(
            'col_num'    => $col_num,
            'start'    => $start,
            'rowset'    => $rowset,
			...
			'NEWEST_POST_IMG'	=> $this->user->img('icon_topic_newest', 'VIEW_NEWEST_POST'),
			'LATEST_POST_IMG'	=> $this->user->img('icon_topic_latest', 'VIEW_LATEST_POST'),
            //'PREV_PAGE'    => $rootref['U_LT_' . $col_num . '_PREVIOUS_PAGE'],
            //'U_OLD_PAGE'    => $u_old_page,
            //'U_CURRENT_PAGE'    => '<span>' .  $rootref['LT_' . $col_num . '_CURRENT_PAGE'] . '</span>',
            //'U_PREV_PAGE'    => $rootref['U_LT_' . $col_num . '_PREVIOUS_PAGE'],
            //'U_NEXT_PAGE'    => $rootref['U_LT_' . $col_num . '_NEXT_PAGE'] ,
        ...
        //return $this->controller_helper->render('lasttopicsajax_body.html');
        //$data = $this->controller_helper->render('lasttopicsajax_body.html');
        ////print_r($data);
                //return $this->controller_helper->render('lasttopicsajax_body.html');
    ...
    private function build_pagination($col_num, $rootref, $dataref)
    {
        ...
        if ($base_url && $total_pages > 6)
        {
		    ...
		    //$str .= '<a href="#" class="dropdown-trigger" title="' . $this->user->lang['JUMP_TO_PAGE_CLICK'] . '" role="button">' .  $rootref['LT_' . $col_num . '_PAGE_NUMBER'] . '</a>';
		    $str .= '<a class="button button-icon-only dropdown-trigger" href="#" title="' . $this->user->lang['JUMP_TO_PAGE_CLICK'] . '" role="button"><i class="icon fa-level-down fa-rotate-270" aria-hidden="true"></i><span class="sr-only">' .  $rootref['LT_' . $col_num . '_PAGE_NUMBER'] . '</span></a>';
			...
			$str .= 	'<input type="number" name="page-number" min="1" max="999999" title="' . $this->user->lang['JUMP_TO_PAGE'] .  '" class="inputbox tiny" data-per-page="'.  $rootref['LT_' . $col_num . '_PER_PAGE'] . '" data-base-url="' .  $rootref['LT_' . $col_num . '_BASE_URL'] . '|e("html_attr")' . '" data-start-name="' .  $rootref['LT_' . $col_num . '_START_NAME'] . '" />';
        ...
        foreach ($pagination_arr as $item)
        {
            if($item['S_IS_PREV'])
            {
                //$str .= 	'<li  data-col-prev="' . $col_num .'" class="previous"><a href="' . $item['PAGE_URL'] .'" rel="prev" role="button">' . $this->user->lang['PREVIOUS'] . '</a></li>';
            ...
            {
                //$str .= 	'<li  data-col-next="' . $col_num .'" class="next"><a href="' . $item['PAGE_URL'] .'" rel="next" role="button">' . $this->user->lang['NEXT'] . '</a></li>';
            ...
            {
               // $str .= 	'<li data-col-pagenum="' . $col_num .'"><a href="' . $item['PAGE_URL'] .'" role="button">' . $item['PAGE_NUMBER'] .'</a></li>';
...
return $str;
    ...
    private function build_topics($col_num, $rowset)
    {
        ...
        foreach ($rowset as $item)
        {
            ...
            {
               // $str .= 	'<a  data-lt1_column="' + $col_num + '" class="topictitle" href="' . $item['U_NEWEST_POST'] . '">' . $newest_post_img . '</a> <a data-lt2_column="2" class="topictitle" href="' . $item['U_NEWEST_POST'] . '" title="' . $item['TITLE'] . '" class="link-new">' . $item['TITLE'] . ' (' . $item['TOPIC_REPLIES'] . ')</a>';
				$str .= 	'<span data-topic_id="' + $item['topic_id'] + '" data-forum_id="' + $item['forum_id'] + '" class="lt-marktopicread" title="' + $this->user->lang['MARK_READ'] + '"><i class="fa fa-check-square-o" aria-hidden="true"></i></span>';
                $str .= 	'<a data-lt1_column="' + $col_num + '" class="unread" href="' . $item['U_NEWEST_POST'] . '">';
				$str .= 	'<i class="icon fa-file fa-fw icon-red icon-md" aria-hidden="true"></i>';
				$str .= 	'</a>';
                $str .= 	'<a data-lt2_column="' + $col_num + '" class="topictitle" href="' . $item['U_NEWEST_POST'] . '">' . $item['TITLE'] . '</a>';
            }
            else
            {
                //$str .= 	'<a  data-lt1_column="2" class="topictitle" href="' . $item['U_LAST_POST'] . '">' . $latest_post_img . '</a> <a data-lt2_column="2" class="topictitle" href="' . $item['U_LAST_POST'] . '" title="' . $item['TITLE'] . '" class="link-new">' . $item['TITLE'] . ' (' . $item['TOPIC_REPLIES'] . ')</a>';
				$str .= 	'<span  style="visibility:hidden;"  data-topic_id="' + $item['topic_id'] + '" data-forum_id="' + $item['forum_id'] + '" class="lt-marktopicread" title="' + $this->user->lang['MARK_READ'] + '"><i class="fa fa-check-square-o" aria-hidden="true"></i></span>';
                $str .= 	'<a data-lt1_column="' + $col_num + '"href="' . $item['U_LAST_POST'] . '">';
				$str .= 	'<i class="icon fa-file fa-fw icon-lightgray icon-md" aria-hidden="true"></i>';
				$str .= 	'</a>';
                $str .= 	'<a data-lt2_column="' + $col_num + '" class="topictitle" href="' . $item['U_LAST_POST'] . '">' . $item['TITLE'] . '</a>';
            }
            ...
            {
			    $str .= 	'<br />';
                //$str .= 	'<div class="topic_preview_content" style="display:none;">';
                //$str .= 	'<strong>' . $this->user->lang['FIRST_POST'] . '</strong><hr>	<div class="topic_preview_avatar  topic_preview_first_avatar">' . $item['TOPIC_PREVIEW_FIRST_AVATAR'] .'</div>	<div class="topic_preview_first">' . $item['TOPIC_PREVIEW_FIRST_POST'] .'</div>';
                //$str .= 	'<div class="topic_preview_break"></div>';
                //$str .= 	'<strong>' . $this->user->lang['LAST_POST'] . '</strong><hr>';
                //$str .= 	'<div class="topic_preview_avatar topic_preview_last_avatar">' . $item['TOPIC_PREVIEW_LAST_AVATAR'] .'</div><div class="topic_preview_last">' . $item['TOPIC_PREVIEW_LAST_POST'] .'</div>';
                //$str .= 	'</div>';

                $str .= 	'<div class="topic_preview_content" style="display:none;">';
			    $str .= 	'<strong>' . $this->user->lang['FIRST_POST']  + '<hr>';
				$str .= 	'<div class="topic_preview_avatar">' . $item['TOPIC_PREVIEW_FIRST_AVATAR'] . '</div>';
		        $str .= 	'<div class="topic_preview_first">' . $item['TOPIC_PREVIEW_FIRST_POST'] .'</div>';
			    $str .= 	'<div class="topic_preview_break"></div>';
		        $str .= 	'<strong>' . $this->user->lang['LAST_POST'] . '</strong><hr>';
				$str .= 	'<div class="topic_preview_avatar">' . $item['TOPIC_PREVIEW_LAST_AVATAR'] .'</div>';
				$str .= 	'<div class="topic_preview_last">' . $item['TOPIC_PREVIEW_LAST_POST'] .'</div>';
	            $str .= 	'</div>';                
            }
            $str .= 	'</li>';
        }
